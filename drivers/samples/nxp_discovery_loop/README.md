...
Chip-specific logic (IRQ handling and LPCD calibration) lives in `src/disc_loop_pn5190.c`.

## Requirements

- [nRF54L15 DK](https://docs.nordicsemi.com/bundle/ncs-latest/page/zephyr/boards/nordic/nrf54l15dk/doc/index.html)
- PNEV5190B evaluation board
...
Pin assignments are defined in `overlay-pn5190.overlay`. The board-specific overlay in `boards/nrf54l15dk_nrf54l15_cpuapp.overlay` only enables GPIO and external flash; it does not configure the NFC interface.
...
> **Note:** The chip-specific overlay disables buttons and LEDs that share pins with the NFC interface (for example P0.04 IRQ and P1.13 SCK).
...
NXP driver options (PAL components, and so on) are configured through the NXP driver Kconfig (`CONFIG_NFC_DRIVER_NXP`, `CONFIG_PN5190_DRV`, etc.). See `drivers/nfc/nxp/Kconfig`.
