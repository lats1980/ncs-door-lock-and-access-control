...
LOG_MODULE_REGISTER(nxp_discovery_loop, CONFIG_NCS_NXP_DISCOVERY_LOOP_SAMPLE_LOG_LEVEL);
...
static volatile uint8_t halt_forever = 1U;
...
static void disc_loop_run(void);
...
int main(void)
{
	...
	do {
		if (nxp_nfc_init() != 0) {
			LOG_ERR("nxp_nfc_init failed");
			ret = -1;
			break;
		}

		LOG_INF("DiscoveryLoop Example");

		nfc_lib_status = phNfcLib_Init();
		if (nfc_lib_status != PH_NFCLIB_STATUS_SUCCESS) {
			LOG_ERR("phNfcLib_Init failed: 0x%04x", nfc_lib_status);
			ret = -1;
			break;
		}
	...
	defined(CONFIG_NFC_NXP_PAL_I14443P4)
		...
		if (nxp_nfc_start_irq_monitor(disc_loop_irq_handler) != 0) {
			LOG_ERR("nxp_nfc_start_irq_monitor failed");
			ret = -1;
			break;
		}
	...
	while (halt_forever) {
		/* Halt on initialization failure or unexpected scheduler exit. */
	}
