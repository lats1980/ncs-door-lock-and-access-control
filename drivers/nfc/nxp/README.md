...
This driver integrates the NXP PN5190 NFC reader IC with nRF Connect SDK / Zephyr by porting the NXP NFC Reader Library platform layers to Zephyr.

It is compatible with **NXP NFC Reader Library 07.16** (`NxpNfcRdLib_07.16.00`).

> **Note:** Only PN5190 is supported. PN5180 is not supported because ISO-DEP transport cannot be guaranteed reliably on that frontend.
...
| **BAL** (Bus Abstraction Layer) | `phbalReg_ZephyrSpi.c` | SPI communication with the PN5190 via Zephyr SPI |
...
- **HAL** (`phhalHw`) — hardware abstraction for PN5190
