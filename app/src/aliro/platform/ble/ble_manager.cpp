...
AliroError BleManager::Init()
{
	...
	L2capServer::Callbacks l2capCallbacks = {
		.mOnConnected =
			[](bt_conn *conn) { AliroStack::Instance().CreateSession(ConnectionHandle::Ble(conn)); },
		.mOnDisconnected =
			[](bt_conn *conn) { AliroStack::Instance().DestroySession(ConnectionHandle::Ble(conn)); },
		.mOnDataReceived =
			[](bt_conn *conn, uint8_t *data, size_t length) {
				AliroStack::Instance().HandleSessionData(ConnectionHandle::Ble(conn), { data, length });
			},
	};
