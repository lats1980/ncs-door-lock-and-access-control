...
/**
 * @file key.h
 * @brief Key derivation for Internal ZMS AEAD operations.
 *
 * Derives per-entry 256-bit encryption keys from the device's Hardware
 * Unique Key (HUK) using the ZMS entry identifier as context, ensuring
...
namespace DoorLock::InternalZms::Key {
 ...
 * device. This must succeed before @ref Derive can be used.
 *
 * @retval 0 on success.
...
/**
 * @brief Derive an encryption key for a given ZMS entry.
 *
 * Uses the Hardware Unique Key to derive a 256-bit key that is unique
 * to the specified @p id.
 *
 * @param      id  ZMS entry identifier used as derivation context.
