...
LOG_MODULE_REGISTER(internal_zms, CONFIG_DOOR_LOCK_INTERNAL_ZMS_LOG_LEVEL);

/** Mutex protecting all internal ZMS operations and internal shared state. */
static K_MUTEX_DEFINE(sInternalZmsMutex);

namespace DoorLock::InternalZms {

namespace {

constexpr size_t kMaxDataSize{ CONFIG_DOOR_LOCK_INTERNAL_ZMS_MAX_DATA_SIZE };

static_assert(kMaxDataSize <= std::numeric_limits<uint16_t>::max(),
	      "CONFIG_DOOR_LOCK_INTERNAL_ZMS_MAX_DATA_SIZE is too big");
...
{
	k_mutex_lock(&sInternalZmsMutex, K_FOREVER);
	const auto ec = InitLocked(partitionId);
	k_mutex_unlock(&sInternalZmsMutex);
	return ec;
}
