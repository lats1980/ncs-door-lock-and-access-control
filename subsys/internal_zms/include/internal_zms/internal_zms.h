...
/**
 * @file internal_zms.h
 * @brief Public API for the Internal ZMS subsystem.
 *
 * Provides encrypted non-volatile storage on the internal RRAM flash of
 * nRF54 series devices using the Zephyr Memory Storage (ZMS) backend.
...
namespace DoorLock::InternalZms {

/** @brief Type used to identify entries stored in internal ZMS. */
using Id = uint16_t;

#ifdef CONFIG_DOOR_LOCK_INTERNAL_ZMS_ROLLBACK_PROTECTION

/** @brief Reserved ID for the Unique ID. */
constexpr Id kUniqueIdReservedId{ 0xFFFF };

#endif // CONFIG_DOOR_LOCK_INTERNAL_ZMS_ROLLBACK_PROTECTION

/**
 * @brief Initialize the Internal ZMS subsystem.
 *
 * Sets up the underlying ZMS storage partition, the nonce generator, and
 * (if enabled) the rollback-protection counter.
 *
 * @param partitionId Flash partition identifier to use for ZMS storage.
 *
 * @retval 0 on success.
...
/**
 * @brief Erase all data from the internal ZMS storage.
 *
 * Clears the rollback-protection counters and wipes the underlying ZMS
...
/**
 * @brief Encrypt and write data to internal ZMS.
 *
 ...
 * deleted.
 *
 * @param id   Identifier for the ZMS entry.
 ...
 *             @c CONFIG_DOOR_LOCK_INTERNAL_ZMS_MAX_DATA_SIZE.
 *
 * @retval 0 on success.
...
/**
 * @brief Read and decrypt data from internal ZMS.
 *
 ...
 *
 * @param[in]     id   Identifier for the ZMS entry.
...
/**
 * @brief Delete an entry from internal ZMS.
 *
 ...
 *
 * @param id Identifier for the ZMS entry to delete.
 *
 * @retval 0 on success.
 * @retval negative errno code on failure.
 */
